    ...
    def _init_graph(self) -> None:
        ...
        with tfutil.absolute_variable_scope(self.scope, reuse=False), tfutil.absolute_name_scope(self.scope):  # ignore surrounding scopes
            ...
            with tf.control_dependencies(None):  # ignore surrounding control dependencies
                self.input_templates = [tf.keras.backend.placeholder(dtype=tf.float32, name=name) for name in self.input_names]
        ...
        if key not in self._run_cache:
            with tfutil.absolute_name_scope(self.scope + "/_Run"), tf.control_dependencies(None):
                with tf.device("/cpu:0"):
                    in_expr = [tf.keras.backend.placeholder(dtype=tf.float32, name=name) for name in self.input_names]
